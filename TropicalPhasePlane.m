function h = TropicalPhasePlane(F, G, dF, dG, ulim, vlim, showLabels)
%%% Options
datacursormode off
...
TL = TropicalCurves(F,G,dF,dG);
T = [F;G];
dT = [dF,zeros(size(dG)); zeros(size(dF)), dG];
nT = size(T,1);
nF = size(F,1);
nG = size(G,1);
    ...
    for j = i+1:nT
        ...
        if lc ~= NULLCLINE
            ...
            for iq = 2:size(Q,2)
                ...
                if length(kmax) > 1 && dTsign(direction) == 0
                    c = [1,1,1];
